...
export class Get {
  ...
  public async directorates(req: Request, res: Response): Promise<void> {
    ...
    const directorates: IDirectorateDocument[] = await directorateService.getAllDirectorates() as IDirectorateDocument[];

    res.status(HTTP_STATUS.OK).json({ message: 'All Directorates', Data: directorates });
  }
  ...
  public async directorate(req: Request, res: Response): Promise<void> {
    ...
    res.status(HTTP_STATUS.OK).json({ message: 'Directorate', Data: directorate, Users: users });
  }
