...
>(({ jsDate, onJsDateChange, showClearButton = true, ...props }, ref) => {
   ...
   const { groupProps, fieldProps, buttonProps: _buttonProps, dialogProps, calendarProps } = useDatePicker(props, state, divRef);
   ...
   return (
      ...
      >
         ...
         <div className={cn('ml-2 mr-2 h-5 w-5', !showClearButton && 'hidden')}>
            <X className={cn('h-5 w-5 cursor-pointer text-primary/30', !jsDatetime && 'hidden')} onClick={() => setJsDatetime(null)} />
         </div>
      </div>
   );
});
