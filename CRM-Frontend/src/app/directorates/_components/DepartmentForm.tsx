...
import { addDepartmentInDirectorateSchema } from '@/schema/organization.schema';
//Icons
import { TriangleAlert } from 'lucide-react';

type addDepartmentData = z.infer<typeof addDepartmentInDirectorateSchema>;
...
const DepartmentForm = ({ department, directorateId }: Props) => {
   ...
   const form = useForm<addDepartmentData>({
      resolver: zodResolver(addDepartmentInDirectorateSchema),
      defaultValues: department
         ...
         : {
              directorateId: directorateId?.toString()
           }
   });
      ...
      try {
         if (department) {
            ...
            if (response.status === 200) {
               ...
               router.push(`/directorates/view/${department.directorateId.toString()}`);
            }
         } else {
            // create department
            ...
            if (response.status === 200) {
               router.push(`/directorates/view/${directorateId}`);
               toast({
                  title: 'تم الإنشاء',
