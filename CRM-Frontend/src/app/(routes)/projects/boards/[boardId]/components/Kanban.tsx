...
import { ChatBubbleIcon, DotsHorizontalIcon, ExclamationTriangleIcon, TrashIcon } from '@radix-ui/react-icons';
...
const Kanban = (props: any) => {
   ...
   const onDragEnd = async ({ source, destination }: DropResult) => {
      ...
      const sourceColIndex = data.findIndex((e: any) => e.id === source.droppableId);
      const destinationColIndex = data.findIndex((e: any) => e.id === destination.droppableId);
   ...
   const createTask = async (sectionId: string) => {
      ...
      try {
         const task = await axios.post(`/api/projects/tasks/create-task/${boardId}`, {
            section: sectionId
         });
   ...
   return (
      <>
         ...
         <div className='overflow-scroll flex flex-col space-y-2  '>
            {/* Dialogs */}
            <Dialog open={sectionOpenDialog} onOpenChange={() => setSectionOpenDialog(false)}>
               <DialogContent>
                  <DialogHeader>
                     <DialogTitle className='p-2'>Create new section</DialogTitle>
                     <DialogDescription className='p-2'>Fill out the form below to create a new section to this project.</DialogDescription>
                  </DialogHeader>
                  <NewSectionForm boardId={boardId} onClose={() => setSectionOpenDialog(false)} />
               </DialogContent>
            </Dialog>
            {/* Dialogs end */}
            {
            ...
            <Sheet open={updateOpenSheet} onOpenChange={() => setUpdateOpenSheet(false)}>
               <SheetContent>
                  <UpdateTaskDialog
                     users={users}
                     boards={boards}
               ...
               <DragDropContext onDragEnd={onDragEnd}>
                  <div className='flex flex-row items-start  '>
                     {data?.map((section: any, index: any) => (
                        <div className='flex flex-col items-center justify-center  h-full w-80 ' key={section.id}>
                           <Droppable key={section.id} droppableId={section.id}>
                              {(provided) => (
                                 <div ref={provided.innerRef} {...provided.droppableProps} className='flex flex-col  w-full h-full px-2 '>
                                    <div className='flex flex-col items-center justify-center py-2   '>
                                       <div className='flex flex-row items-center justify-between w-full border '>
                                          ...
                                          <div className='flex items-center justify-end pr-2'>
                                             <span className='border rounded-full px-2 m-2'>{section?.tasks?.length}</span>
                                          ...
                                          <Draggable key={task.id} draggableId={task.id} index={index}>
                                             {(provided: any, snapshot: any) => (
                                                ...
                                                >
                                                         ...
                                                         {task?.dueDateAt &&
                                                            ...
                                                            task.dueDateAt < Date.now() && (
                                                               <HoverCard>
                                                                  ...
                                                                  <HoverCardContent>Attention! This task is overdue!</HoverCardContent>
                                                               </HoverCard>
                                                            )}
                                                         {task.taskStatus === 'COMPLETE' && (
                                                            <HoverCard>
                                                   ...
                                                   <div className='py-1'>Due date: {moment(task.dueDateAt).format('YYYY-MM-DD')}</div>
                                                   <div className='my-2'>
                                                      <p
                                                         className={
                                                   ...
                                                   <HoverCard>
                                                      <HoverCardTrigger className='line-clamp-2 mb-2'>{task.content}</HoverCardTrigger>
