...
const TaskViewActions = ({ taskId, users, boards, initialData }: { taskId: string; users: any; boards: any; initialData: any }) => {
   ...
   return (
      <div className='space-x-2 pb-2'>
         ...
         {initialData.taskStatus !== 'COMPLETE' && (
            <Badge variant={'outline'} onClick={onDone} className='cursor-pointer' aria-disabled={isLoading}>
               <CheckSquare className='w-4 h-4 mr-2' />
               {isLoading ? <Icons.spinner className='animate-spin w-4 h-4 mr-2' /> : 'Mark as done'}
            </Badge>
         )}
         <Badge variant={'outline'} className='cursor-pointer' onClick={() => setOpenEdit(true)}>
            <Pencil className='w-4 h-4 mr-2' />
            Edit
         </Badge>
         <Sheet open={openEdit} onOpenChange={() => setOpenEdit(false)}>
            <SheetContent>
               <UpdateTaskDialog users={users} boards={boards} initialData={initialData} onDone={() => setOpenEdit(false)} />
